...
class Main extends PluginBase implements Listener{
	...
	public function onPlayerTouch(PlayerInteractEvent $event) {
		...
		if($event->getBlock()->getID() === 63 || $event->getBlock()->getID() === 68){ 
			...
			if(TextFormat::clean($sign->getText()[0]) === '[Earn Reward]'){
				...
				foreach($this->getConfig()->get("reward-commands") as $command){
					$player->getServer()->dispatchCommand(new ConsoleCommandSender(), str_ireplace("{PLAYER}", $player->getName(), $command));
				}
	...
	public function onQuit(PlayerQuitEvent $event) {
		...
		if($this->getConfig()->get("RemovePlayerDataOnQuit")) {
			$this->data->remove($name, array($player->x, $player->y, $player->z, $player->getLevel()->getName()));
			$this->data->save();
		}
	}
